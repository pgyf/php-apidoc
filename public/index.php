...
$projectName = \PhalApi\DI()->config->get('app.api_name', '接口文档');//\PhalApi\T('PhalApi API Framework');
$docViewCode = \PhalApi\DI()->config->get('app.api_pwd'); // 查看文档密码，为空时不限制
...
if (substr(PHP_SAPI, 0, 3) == 'cli') {
    ...
    // 接口详情页
    $apiDesc = new core\ApiDesc($projectName);
    $apiDesc->render($detailTpl);
} else {
    checkViewCode();

    // 接口列表页
    $apiList = new core\ApiList($projectName);
