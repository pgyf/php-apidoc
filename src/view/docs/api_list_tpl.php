            ...
            <div class="four wide column">
                ...
                <?php
                    ...
                    foreach ($allApiS as $namespace => $subAllApiS) {
                        ...
                        $namespaceService = $allApiNames[$namespace];//str_replace('\\', '_', trim($namespace, '\\'));
                        echo sprintf('<h4 class="title active" style="font-size:16px;margin:0px;"><i class="dropdown icon"></i>%s (%d)</h4>', $namespaceService, $subMethodTotal);
                    ...
                    <div class="ui  tab <?php if ($num2 == 0) { ?>active<?php } ?>" data-tab="<?php echo str_replace('\\', '_', $namespace) . $key; ?>">
                        <table
                            ...
                            foreach ($item['methods'] as $mKey => $mItem) {
                                ...
                                $NO   = $num++;
                                echo "<tr><td>{$NO}</td><td><a href=\"$link\" target='_blank'>{$s}</a> <br/> {$apiUrl} </td><td>{$mItem['title']}</td><td>{$mItem['desc']}</td></tr>";
                            }
                            ?>
                            </tbody>
                        </table>
